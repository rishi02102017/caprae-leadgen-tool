...
# User input
query = st.text_input("Enter your lead category (e.g., 'data tools', 'marketing')", "")
no_filter = st.checkbox("Show all leads without filtering")
...
if st.button("Scrape Leads"):
    ...
    df = scrape_books(pages=3)

    # Show all leads
    if no_filter or query.strip() == "":
        st.success("✅ Scraping complete! Showing all leads:")
        filtered_df = df
    else:
        query = query.lower()
        filtered_df = df[df["Lead Name"].str.lower().str.contains(query)]
    ...
    st.write("### Lead Scoring Summary")
    st.dataframe(filtered_df[["Lead Name", "Lead Value (£)", "Engagement Status", "Lead Score"]])
